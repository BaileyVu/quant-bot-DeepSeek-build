...
_DEFAULT_PRESETS: Dict[str, _SymbolPreset] = {
    "BTC": _SymbolPreset(2, 3, 0.1, 0.001, 10.0, 25.0),
    "ETH": _SymbolPreset(2, 3, 0.01, 0.001, 10.0, 25.0),
}


class HyperliquidAdapter(PrecisionMixin):
    name = "hyperliquid"
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._account = AccountState(equity=15_000.0, available_balance=15_000.0, timestamp=utc_now())
    ...
    async def _fetch_symbol_meta(self, symbol: str) -> SymbolMeta:
        ...
        if not preset:
            preset = list(_DEFAULT_PRESETS.values())[0]
        return SymbolMeta(
            symbol=symbol.upper(),
    ...
    async def place_order(self, request: OrderRequest) -> PlacedOrder:
        async with self._lock:
            ...
            if existing:
                existing.qty += qty * direction
                if price:
                    existing.entry_price = price
                entry_ref = existing.entry_price
            else:
    ...
    async def cancel_order(self, order_id: str) -> None:  # pragma: no cover - not used in tests
        await self._rate_limiter.acquire()
        return None
